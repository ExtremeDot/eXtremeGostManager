...
DEFAULT_IPERF_PORT="33000"
DEFAULT_IPERF_DURATION="10"
...
speed_test() {
    ...
    if [ "$SERVER_ROLE" == "foreign" ]; then
        echo "Running iperf3 server on port $IPERF_PORT ..."
        iperf3 -s -p "$IPERF_PORT"
        return $?
    fi

    echo "Starting tunnel speed test to 127.0.0.1:$IPERF_PORT ..."
    echo ""

    result=$(iperf3 -c 127.0.0.1 \
        -p "$IPERF_PORT" \
        -t "$IPERF_DURATION" \
        -P "$IPERF_PARALLEL" \
        -f m 2>&1)
    ...
    if [ $status -ne 0 ]; then
        echo -e "\033[31mSpeed test failed (iperf3 exit code $status)\033[0m"
        echo "$result" | tail -n 15
        return 1
    fi
    ...
    # استخراج سرعت نهایی receiver و sender
    receiver_speed=$(echo "$result" | awk '
        $0 ~ /\[SUM\]/ && $0 ~ /receiver/ {
            for (i=NF; i>=1; i--) {
                if ($i ~ /^[0-9]+(\.[0-9]+)?$/) {
                    print $i
    ...
    receiver_speed=${receiver_speed:-0}
    sender_speed=${sender_speed:-0}

    if [ "$receiver_speed" = "0" ] || [ -z "$receiver_speed" ]; then
        echo -e "\033[31mCould not parse receiver speed\033[0m"
    ...
    echo "Final Results:"
    #printf "  %-18s : ${ORANGE}%6.1f Mbit/s\n" "Upload (sender)"   "$sender_speed"
    #printf "  %-18s : ${CYAN}%6.1f Mbit/s\n" "Download (receiver)" "$receiver_speed"
    #echo -e "${NC}"


    # طبقه‌بندی جدید
    # ─── Classification based on download speed (receiver) ───
    speed="$receiver_speed"

    if   (( $(echo "$speed < 60"  | bc -l) )); then
        grade="Very Weak"
        score=25
        bar="▁▁▁▁▁░░░░░░░░░"
    elif (( $(echo "$speed < 120" | bc -l) )); then
        grade="Fair / Below Average"
        score=45
        bar="██▌░░░░░░░░░░░"
    elif (( $(echo "$speed < 240" | bc -l) )); then
        grade="Average"
        score=60
        bar="████▌░░░░░░░░░"
    elif (( $(echo "$speed < 480" | bc -l) )); then
        grade="Good"
        score=80
        bar="███████▒░░░░░"
    elif (( $(echo "$speed < 640" | bc -l) )); then
        grade="Very Good"
        score=92
        bar="████████▌░░░░"
    elif (( $(echo "$speed < 960" | bc -l) )); then
        grade="Excellent"
        score=98
        bar="█████████░░░░"
    else
    ...
    echo -e "${YELLOW}Upload Speed     : ${sender_speed} Mbit/s${NC}"
	echo -e "${ORANGE}Download Speed   : ${speed} Mbit/s${NC}"
    echo -e "${GREEN}Classification   : $grade${NC}"
    echo -e "${GREEN}Score            : $score / 100${NC}"
    echo -e "${CYAN}Tunnel Quality   : $bar${NC}"
    echo "----------------------------------------"
}
...
configure_speed_test() {

source /etc/gost-manager.conf

echo ""
echo "Tunnel Speed Test Configuration"
echo "--------------------------------"
echo "Choose a preset based on server network speed:"
echo ""
echo "1) 1 Gbps Server  (Recommended for most VPS)"
echo "2) 4 Gbps Server"
echo "3) 10 Gbps+ Server"
...
IPERF_DURATION=10
IPERF_PARALLEL=4
read -p "Port [$IPERF_PORT]: " new_port
IPERF_PORT=${new_port:-$IPERF_PORT}
;;
...
read -p "Port [$IPERF_PORT]: " new_port
read -p "Duration seconds [$IPERF_DURATION]: " new_time
read -p "Parallel streams [$IPERF_PARALLEL]: " new_parallel

IPERF_PORT=${new_port:-$IPERF_PORT}
IPERF_DURATION=${new_time:-$IPERF_DURATION}
IPERF_PARALLEL=${new_parallel:-$IPERF_PARALLEL}
...
if [ "$SERVER_ROLE" == "foreign" ]; then
    ...
    echo ""
    echo -e "${CYAN}Make sure port [2087] is already forwarded to this server."
    echo -e "${CYAN}Running tunnel listener on the foreign server."
    echo -e "${ORANGE}Press Ctrl+C to stop the test.${NC}"
    echo "------------------------------------------------"
    echo ""
    ...
    echo ""
    echo -e "${CYAN}Checking tunnel connectivity (timeout: 5 seconds)...${NC}"
    echo "------------------------------------------------"

    response=$(curl -s --max-time 5 127.0.0.1:2087)
    status=$?

    if [ $status -eq 0 ] && [ -n "$response" ]; then
        echo -e "${GREEN}Tunnel is working correctly.${NC}"
        echo "Public IP from foreign server: $response"
    else
        echo -e "${RED}Tunnel check failed!${NC}"
        echo ""
        echo -e "${ORANGE}Troubleshooting:${NC}"
        echo "1) Make sure this option is running on the Foreign (Kharej) server."
        echo "2) Verify port 2087 is forwarded correctly."
        echo "3) Check firewall rules on both servers."
        echo "4) Ensure no other service is using port 2087."
        return 1
    fi
...
printf "%-55s %-55s\n" \
"21) WSS-D  | WebSocket Secure - Direct" \
"22) WSS-R  | WebSocket Secure - Reverse "

printf "%-55s %-55s\n" \
"23) WS-D   | WebSocket Plain - Direct" \
